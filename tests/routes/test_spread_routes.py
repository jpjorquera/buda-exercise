...
from tests.mocks.spread_mock import MARKET_SPREAD_ALERT_MOCK, MARKETS_SPREAD_MOCK
...
def test_get_market_spread_with_set_alert_success():
    with patch(
        "app.routes.spread_routes.calculate_spread",
...
def test_get_market_spread_alert_success():
    with patch(
        "app.routes.spread_routes.get_market_spread_alerts",
        return_value=MARKET_SPREAD_ALERT_MOCK,
    ):
        response = client.get(f"/spreads/{MARKET_ID_MOCK}/alert")
        assert response.status_code == status.HTTP_200_OK
        assert response.json() == MARKET_SPREAD_ALERT_MOCK


def test_get_market_spread_alert_not_found_error():
    with patch(
        "app.routes.spread_routes.get_market_spread_alerts",
        side_effect=SpreadAlertNotFoundError(MARKET_ID_MOCK),
    ):
        response = client.get(f"/spreads/{MARKET_ID_MOCK}/alert")
        assert response.status_code == status.HTTP_404_NOT_FOUND


def test_get_market_spread_alert_unexpected_error():
    with patch(
        "app.routes.spread_routes.get_market_spread_alerts",
